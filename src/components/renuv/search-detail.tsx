...
import { TrendingUp, TrendingDown, ArrowLeft, Search, CheckCircle2 } from 'lucide-react';
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(94,168,255,0.18),transparent_32%),linear-gradient(180deg,#eef5fb_0%,#f7f9fc_58%,#edf3f9_100%)] text-[var(--ink-950)]">
      <div className="mx-auto max-w-[1680px] px-6 py-8 md:px-8 lg:px-10">
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <h1 className="mt-3 max-w-4xl text-2xl font-semibold tracking-[-0.04em] text-[var(--ink-950)] md:text-4xl lg:text-5xl">
                {snapshot.brand} · Search performance
              </h1>
        ...
        <section className="mb-6 rounded-[28px] border border-[var(--line-soft)] bg-[rgba(255,255,255,0.78)] p-6 shadow-[0_24px_70px_rgba(19,44,74,0.08)] backdrop-blur">
          <SectionHeading eyebrow="Top performers" title="Highest-impact search terms" icon={<Search size={20} />} />
          ...
          <div className="mt-5 space-y-3">
            {snapshot.topTerms.map((term, idx) => (
              <SearchTermCard key={idx} term={term} rank={idx + 1} />
            ))}
          </div>
        </section>

        <section className="mb-6 rounded-[28px] border border-[var(--line-soft)] bg-[rgba(255,255,255,0.78)] p-6 shadow-[0_24px_70px_rgba(19,44,74,0.08)] backdrop-blur">
          <SectionHeading eyebrow="Analysis" title="Commentary & insights" icon={<Search size={20} />} />
...
function SearchTermCard({ term, rank }: { term: SearchTermPerformance; rank: number }) {
  const categoryConfig = {
    brand: { label: 'Brand', color: 'bg-[var(--blue-700)] text-white' },
    category: { label: 'Category', color: 'bg-[rgba(94,168,255,0.12)] text-[var(--blue-700)]' },
    competitor: { label: 'Competitor', color: 'bg-[#fff0e8] text-[#b15d27]' },
    recovery: { label: 'Recovery', color: 'bg-[#e7f4ee] text-[#2d8a56]' },
    'post-surgery': { label: 'Post-Surgery', color: 'bg-[rgba(147,108,230,0.12)] text-[#8860d8]' }
  }[term.category];
  ...
  return (
    <div className="rounded-[22px] border border-[var(--line-soft)] bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--panel-muted)] text-xs font-semibold text-[var(--ink-700)]">
            {rank}
          </span>
          <div>
            <h3 className="text-base font-semibold text-[var(--ink-950)]">{term.term}</h3>
            <div className="mt-2 flex flex-wrap gap-2">
              <span className={cn('rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.14em]', categoryConfig.color)}>
              ...
              <span className="rounded-full border border-[var(--line-soft)] bg-[var(--panel-muted)] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--ink-600)]">
                ${(term.sales / 1000).toFixed(1)}k sales
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <MetricPill label="CTR" value={`${term.ctr.toFixed(2)}%`} />
        <MetricPill label="Clicks" value={term.clicks.toString()} />
        <MetricPill label="Orders" value={term.orders.toString()} />
        <MetricPill label="CVR" value={`${term.conversionRate.toFixed(1)}%`} />
      </div>
    </div>
  );
}
...
function MetricPill({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[16px] border border-[var(--line-soft)] bg-[var(--panel-muted)] p-3">
      ...
      <p className="mt-1 text-sm font-semibold text-[var(--ink-950)]">{value}</p>
    </div>
  );
}
