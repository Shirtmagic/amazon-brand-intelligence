    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(94,168,255,0.18),transparent_32%),linear-gradient(180deg,#eef5fb_0%,#f7f9fc_58%,#edf3f9_100%)] text-[var(--ink-950)]">
      <div className="mx-auto max-w-[1680px] px-6 py-8 md:px-8 lg:px-10">
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,420px)] xl:items-start">
            <div className="space-y-5">
              <div className="flex flex-wrap items-center gap-3 text-xs text-[var(--ink-700)]">
                ...
                <Badge tone="blue">ASIN performance</Badge>
              ...
              <div>
                ...
                <h1 className="mt-3 max-w-4xl text-2xl font-semibold tracking-[-0.04em] text-[var(--ink-950)] md:text-4xl lg:text-6xl">
                  ASIN performance · {snapshot.brand}
                </h1>
                <p className="mt-4 max-w-3xl text-base leading-7 text-[var(--ink-700)] md:text-lg">
                  Internal-only product performance surface for ranking the catalog, spotting movers, and flagging where traffic, conversion, or paid dependency is starting to distort the lineup.
                </p>
              </div>

              <div className="flex flex-wrap gap-2 text-xs text-[var(--ink-600)]">
                <Tag>Internal-only</Tag>
                <Tag>Read-only ASIN review</Tag>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Link href={internalRoute(brand, "retail-health")} className="mc-btn mc-btn-secondary">
                  Open retail health <ArrowUpRight size={15} />
                </Link>
              </div>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
              <MiniSummaryCard
                ...
                value="ASIN performance"
                detail="ASIN-level performance data sourced from reporting_amazon views."
                icon={<Database size={18} />}
              />
              <MiniSummaryCard
                title="Trust posture"
                value="Operationally usable"
                detail="Source labels visible per block for auditability."
                icon={<ShieldCheck size={18} />}
              />
              <MiniSummaryCard
                title="Operator focus"
                value="Concentration + CVR"
                detail="Watch hero-SKU dependency and conversion slippage in traffic-heavy ASINs."
        ...
        <section className="mb-6 grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
          <Panel>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <SectionHeading eyebrow="Catalog ranking" title="Ranked ASIN performance" />
                <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--ink-700)]">
                  Product-level leaderboard for the current review window, with enough structure to wire directly to a daily ASIN performance view later.
                </p>
              </div>
            ...
            <div className="mt-5 overflow-hidden rounded-[24px] border border-[var(--line-soft)] bg-white shadow-[0_18px_42px_rgba(19,44,74,0.05)]">
              <div className="hidden md:block overflow-x-auto">
                <table className="min-w-full border-collapse text-left">
                  <thead className="bg-[var(--panel-muted)]">
                    <tr>
                      {['ASIN', 'Product', 'Category', 'Revenue', 'Units', 'CVR', 'Ad share', 'Trend', 'Flags'].map((column) => (
                        <th key={column} className="px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--ink-600)]">
                          {column}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {snapshot.topAsins.map((row) => (
                      <tr key={row.asin} className="border-t border-[var(--line-soft)] align-top">
                        <td className="px-4 py-4 text-sm font-semibold text-[var(--ink-900)]">{row.asin}</td>
                        <td className="px-4 py-4 text-sm leading-6 text-[var(--ink-800)]">
                          <div className="font-semibold text-[var(--ink-900)]">{row.title}</div>
                        </td>
                        <td className="px-4 py-4 text-sm text-[var(--ink-800)]">{row.category}</td>
                        <td className="px-4 py-4 text-sm font-semibold text-[var(--ink-900)]">{row.orderedRevenue}</td>
                        <td className="px-4 py-4 text-sm text-[var(--ink-800)]">{row.units}</td>
                        <td className="px-4 py-4 text-sm text-[var(--ink-800)]">{row.conversionRate}</td>
                        <td className="px-4 py-4 text-sm text-[var(--ink-800)]">{row.adAttributedShare}</td>
                        <td className="px-4 py-4 text-sm"><ToneBadge tone={row.tone}>{row.revenueTrend}</ToneBadge></td>
                        <td className="px-4 py-4 text-sm leading-6 text-[var(--ink-800)]">
                          <div className="flex flex-wrap gap-2">
                            {row.flags.map((flag) => (
                              <Tag key={`${row.asin}-${flag}`}>{flag}</Tag>
                            ))}
                          </div>
                        </td>
                      </tr>
              ...
              <div className="md:hidden divide-y divide-[var(--line-soft)]">
                {snapshot.topAsins.map((row) => (
                  <div key={row.asin} className="space-y-2 p-4">
                    <div className="flex justify-between gap-3"><span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--ink-600)]">ASIN</span><span className="text-right text-sm font-semibold text-[var(--ink-900)]">{row.asin}</span></div>
                    <div className="flex justify-between gap-3"><span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--ink-600)]">Product</span><span className="text-right text-sm text-[var(--ink-800)]">{row.title}</span></div>
                    <div className="flex justify-between gap-3"><span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--ink-600)]">Revenue</span><span className="text-right text-sm font-semibold text-[var(--ink-900)]">{row.orderedRevenue}</span></div>
                    <div className="flex justify-between gap-3"><span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--ink-600)]">CVR</span><span className="text-right text-sm text-[var(--ink-800)]">{row.conversionRate}</span></div>
                    <div className="flex justify-between gap-3"><span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--ink-600)]">Trend</span><span className="text-right"><ToneBadge tone={row.tone}>{row.revenueTrend}</ToneBadge></span></div>
                  </div>
                ))}
              </div>
              <p className="border-t border-[var(--line-soft)] bg-[var(--panel-muted)] px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--blue-700)]">
                {snapshot.topAsins[0]?.sourceView}
              </p>
            </div>
          </Panel>

          <Panel>
            <SectionHeading eyebrow="Dependency signal" title="Concentration + catalog resilience" />
            <div className="mt-5 rounded-[24px] border border-[rgba(94,168,255,0.14)] bg-[linear-gradient(135deg,rgba(94,168,255,0.12),rgba(255,255,255,0.96))] p-5">
              <h3 className="text-xl font-semibold tracking-[-0.03em] text-[var(--ink-950)]">{snapshot.concentration.headline}</h3>
              <p className="mt-3 text-sm leading-6 text-[var(--ink-700)]">{snapshot.concentration.summary}</p>
            </div>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {snapshot.concentration.metrics.map((metric) => (
                <div key={metric.label} className="rounded-[20px] border border-[var(--line-soft)] bg-white p-4 shadow-[0_12px_30px_rgba(19,44,74,0.05)]">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-[var(--ink-900)]">{metric.label}</p>
                    <ToneBadge tone={metric.tone}>{metric.value}</ToneBadge>
                  </div>
                  <p className="mt-3 text-sm leading-6 text-[var(--ink-700)]">{metric.detail}</p>
                </div>
              ))}
            </div>
            <p className="mt-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--blue-700)]">{snapshot.concentration.sourceView}</p>
          </Panel>
        </section>

        <section className="mb-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)]">
          <Panel>
            <SectionHeading eyebrow="Movers" title="Winners, decliners, and paid-dependency watch" />
            <div className="mt-5 grid gap-3">
              {snapshot.movers.map((item) => (
                <div key={item.label} className="rounded-[22px] border border-[var(--line-soft)] bg-[var(--panel-muted)] p-4">
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--ink-600)]">{item.label}</p>
                        <Tag>{item.asin}</Tag>
                      </div>
                      <p className="mt-2 text-base font-semibold text-[var(--ink-950)]">{item.metric}</p>
                      <p className="mt-2 text-sm leading-6 text-[var(--ink-700)]">{item.detail}</p>
                    </div>
                    <TrendPill trend={item.trend}>{item.change}</TrendPill>
                  </div>
                  <div className="mt-4 flex items-center justify-between gap-3">
                    <ToneBadge tone={item.tone}>{formatToneLabel(item.tone)}</ToneBadge>
                    <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--blue-700)]">{item.sourceView}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <Panel>
            <SectionHeading eyebrow="Operator flags" title="Conversion, traffic, and pressure signals" />
            <div className="mt-5 space-y-3">
              {snapshot.flags.map((flag) => (
                <div key={`${flag.asin}-${flag.issue}`} className="rounded-[22px] border border-[var(--line-soft)] bg-white p-4 shadow-[0_12px_30px_rgba(19,44,74,0.05)]">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-[var(--ink-950)]">{flag.title}</p>
                      <p className="mt-1 text-xs text-[var(--ink-600)]">{flag.asin}</p>
                    </div>
                    <ToneBadge tone={flag.tone}>{flag.issue}</ToneBadge>
                  </div>
                  <p className="mt-3 text-sm leading-6 text-[var(--ink-700)]">{flag.diagnosis}</p>
                  <div className="mt-4 rounded-[18px] border border-[var(--line-soft)] bg-[var(--panel-muted)] p-3">
                    ...
                    <p className="mt-2 text-sm leading-6 text-[var(--ink-800)]">{flag.actionBias}</p>
                  </div>
                  <p className="mt-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--blue-700)]">{flag.sourceView}</p>
                </div>
              ))}
            </div>
          </Panel>
        ...
        <section className="rounded-[28px] border border-[var(--line-soft)] bg-[rgba(255,255,255,0.78)] p-5 shadow-[0_24px_70px_rgba(19,44,74,0.08)] backdrop-blur md:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <SectionHeading eyebrow="Data sources" title="ASIN reporting views" />
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            {Object.entries(renuvAsinContracts).map(([key, source]) => (
